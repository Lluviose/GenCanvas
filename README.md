...
- **一图一节点**：并发生成会生成多个子节点（例如 4 并发 = 4 个子节点），方便对比与继续迭代
- **生成 / 重新生成**：生成会追加批次；重新生成只保留最新批次（旧批次自动隐藏）
- **一键继续**：从任意节点继续生成，自然形成分叉树
- **快速微调**：输入变化词（如"插画风/特写/雨夜"）回车即分叉
- **批量分叉**：多行输入批量创建分支，支持并发生成
- **图片分支**：基于某张生成结果继续探索（支持多轮对话模式）
- **收起与裁剪**：可收起子树并在父节点预览子树图片；可设置仅显示最新 N 级（从叶子向上裁剪）缓解超大画布卡顿
...
- [x] 版本历史与回滚
- [x] 本地/WebDAV 备份
- [x] **Apple 风格 UI 重设计**：玻璃态面板、圆角优化、微交互动效、Dynamic Island 风格通知
